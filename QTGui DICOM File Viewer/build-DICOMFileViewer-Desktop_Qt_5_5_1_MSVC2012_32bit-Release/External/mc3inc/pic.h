 ...
 * PICTools ProductVersion 2.00.636
 * $Header: /PEGASUS/INCLUDE/pic.h 586   2/01/10 9:26a Buildmaster $
 * $Nokeywords: $
 */

/***************************************************************************\
*       Copyright (C) 1996-2012 Accusoft Corporation                        *
*       All rights reserved.                                                *
*****************************************************************************
*       Revision History:                                                   *
...
#define PT_PRGBA     (0x24)     /* Alpha-Multiplied Components */
#define PT_N8CHAN     (0x1A)    /* N-8bit-Component, where n is in range 3 to 8 */
#define PT_N8CHANA    (0x1B)    /* N-8bit Component + Alpha, where n is in range 3 to 8 */
#define PT_N16CHAN    (0x1C)    /* N-16bit-Component, where n is in range 3 to 8 */
#define PT_N16CHANA   (0x1D)    /* N-16bit Component + Alpha, where n is in range 3 to 8 */
...
#define PT_PRGBA_FLOAT  (PTSR_FLOAT|PT_PRGBA)
...
#define PF_AutoBadJAIThresholds     0x00080000L
        /* (JLSE) Auto fix JAI bad default thresholds */
...
#define PF_BadJAIThresholds         0x00200000L
        /* (JLSE) Use JAI bad default thresholds instead of correct standard ones */
...
/*#define PF2_ARMNeonDisable      (0x20L)        \\ disable use of asm Neon optimizations for ARM chip (overloads P2AndP3OptDisable, without harm) */
...
/*#define PF2_ARMAsmDisable      (0x100L)        \\ disable use of asm optimizations for ARM chip (overloads MMXDisable, without harm) */
...
/*#define PF2_ASMDisable_All          (0x00000960L)    \\ disable all assembly optimizations
                                                       \\ equivalent to: ( PF2_MMXDisable | PF2_P2AndP3OptDisable |
                                                       \\                  PF2_P3OptDisable | PF2_P4OptDisable ) */
...
#if defined(__cplusplus) && !defined(STATIC_OPCODE)
  typedef struct {
        BYTE        reserved;
  } WORK_AREA;
#elif defined(PIC64) && !defined(__SPARC__)
#if defined (__sun__) || defined (__AIX64__)
  #pragma pack(8)
#else
  #pragma pack(push, 8)
#endif
  typedef struct {
        jmp_buf_old AppStateOld; /* not used */
        jmp_buf_old PicStateOld; /* not used */
...
typedef struct {
        ...
        PICFLAGS     PicFlags2;     /* PF2_ flags above when implemented (set appropriately for J2K files) */
        ...
        WORD         NumPartitions;   /* For a JPEG2000 image; The number of partitions that is required to extract 
                                         the components of the image. Set appropriately depending upon whether there are 
                                         multiple gray components, or RGB components (which are combined into single 
                                         partitions), or 3D slices (where PF2_3D_Slices is set in PicFlags2)  */
...
typedef struct {
        ...
        LONG    MaxRqtblSize; /* only applies to OP_REQUANT */ 

        /* The next ones apply only to OP_REQUANT.  Passthrough mode for progressives means spectral selection scans are passed through,and those 
             successive approximation scans that are deemed needed for adequate quality are passed through also. */
        DWORD ProgressiveOptions; /* 0 = always use requant (the old method) for progressives.
                                     1 = use passthrough mode for progressives if the first scan seen skips at least one bit.  If the first scan does
                                        not skip at least one bit, buffer the dc scan(s) until either we reach a dc scan that does skip a bit (in case
                                        of non-interleaved dc scan), or the first ac scan header is reached, if that happens
                                        without exceeding ProgressivePassThruMaxDCScanBufferSize described below.  If that first ac scan (or an earlier
                                        dc scan) is seen to skip at least one bit, or if we never reach it because buffer size was exceeded, 
                                        use passthrough mode. Otherwise revert to using requant method rather than passthrough method. 
                                     2 = always use passthrough mode for progressives (which means if there are no succ approx scans, there is no reduction).
                                         The recommended method is 1. When there are no bits dropped from first scan, typically there are no bits
                                     dropped from later scans (occasionally 1 bit) and in this case requant works well and allows for reduction
                                     similar to the sequential case.  */
        DWORD ProgressivePassThruMaxSkipBitsLum;  /* Can be 0,1, or 2, Recommended is 2 (this gives the most flexibility to decide based on the image). */
        DWORD ProgressivePassThruMaxSkipBitsChrom; /* Can be 0,1, or 2, Recommended is 2  (this gives the most flexibility to decide based on the image). */
        LONG  ProgressivePassThruLumFactorReq;  /* 0,...255.  This is used only in the decision process for progresive passthrough mode, to decide which scans to
                                                keep.  By experiment we find that to get size-quality results for progressive passthrough mode that are 
                                                similar to sequential requant, this needs to be about 32 when LumFactorReq for sequential requant is 20 (a
                                                rather small number).  When LumFactorReq for sequential requant is a number larger than 32, it may not 
                                                be necessary to make this number for progressive passthrough mode larger than the sequential number. */
        LONG  ProgressivePassThruChromFactorReq; /* 0,...255.  Same comment as above. When ChromFactorReq for sequential is 48, we got similar results in
                                      progressive passthrough when this number was also 48. */
        LONG  ProgressivePassThruMaxDCScanBufferSize; /* When using ProgressiveOptions = 1, if the first scan (which is DC) does not skip any bits, then
                                                      buffer scans until possibly as far as through the first AC scan header, if that happens without the memory required
                                                      exceeding this number.  This number may be set to zero to not allow buffering dc scan(s). */
        } REORIENT;
...
#define DETECTION_METHOD_NONE       (0)
#define DETECTION_METHOD_HEADER     (1)
#define DETECTION_METHOD_HEADERLESS (2)
...
typedef struct {
		...
		DWORD        DetectionMethod;  /* See DETECTION_METHOD_* values above */
    } CAMERARAWE;
...
typedef struct {
        ...
        DWORD        AlphaStride;          /* Set by PNGE during REQ_INIT to the stride needed to contain an AlphaChannel */
...
#define PF2_ARMNeonDisable      (0x20L)     /* disable ARM chip Neon optimizations (there is no harm in this overloading) */
#define PF2_P3OptDisable        (0x40L)     /* disable P3 optimizations */
#define PF2_MMXDisable          (0x100L)    /* disable MMX, P2 and P3 optimizations */
#define PF2_ARMAsmDisable       (0x100L)    /* disable ARM chip asm optimizations (there is no harm in this overloading) */
...
#define PF2_ForceWSQv2Compliance (0x1L) /* WSQ's spec decides to exclude a wavelet
    subband if a central subrectangle of the subband coefficients (roughly equivalent
    to a subrectangle of the pixels) variance is less than 1.01.  The subrectangle is
    3/4 horizontally and 7/16 vertically. So if the significant fingerprint data are
    in the top or bottom 1/4 of the image, that data is not used in the decision to
    exclude.  If the subrectangle is mostly smoothly one intensity, then enough of
    the wavelet subbands are excluded so that higher resolution detail about the
    fingerprints is not present no matter what bit rate is selected.  For a
    WSQ-spec-compliant image, compression must occur in this way.  But, catch-22,
    because of the resulting fingerprint quality resulting from the missing high
    resolution detail, the FBI can't use the compressed image.  This flag forces
    spec-compliant compression even when for images when this occurs.  Otherwise
    in cases like this, the entire subband rectangle is used in the decision instead
    so it doesn't matter where the fingerprint occurs in the image.  Whether this
    flag is set, or not set, the FBI compliance test is still passed. */
...
typedef struct {
            ...
            PICXWORD        NumPages;       /* Number of pages to insert or delete */
} TIFF_EDIT;
...
#define SCANFIX_LINE_ORIENTATION_HORIZONTAL  0x01
#define SCANFIX_LINE_ORIENTATION_VERTICAL    0x02

typedef struct
{
      LONG X;                 // coordinates of top/left end
      LONG Y;
      LONG Length;            // line length
      LONG Thickness;         // line thickness
      LONG Orientation;       // horizontal or vertical line
      BOOL DeleteLine;        // return true to delete the object
} LineReportData;

typedef struct
{
      LONG X;                 // top left of comb bounding box
      LONG Y;
      LONG Width;             // width of comb (length of base line)
      LONG Height;            // height of comb (tooth height)
      LONG Spacing;           // average spacing of teeth
      LONG Teeth;             // number of teeth found (w / spacing
                              //   gives expected number)
      LONG Confidence;        // confidence of detection, 0 to 100
      BOOL DeleteComb;        // return true to delete the object
} CombReportData;

typedef struct
{
      LONG X;                 // top left of blob bounding box
      LONG Y;
      LONG Width;             // width of bounding box
      LONG Height;            // height of bounding box
      LONG Density;           // pixel count / convex hull area
      LONG Pixels;            // count of pixels in blob
      LONG PointCount;        // number of points in the convex hull
      SFPoint *ConvexHull;    // pointer a list of (x, y) points
                              //  forming the convex hull
      BOOL DeleteBlob;        // return true to delete the object
} BlobReportData;
...
typedef struct
{
      ...
      union
      {
            LineReportData LineData;
            CombReportData CombData;
            BlobReportData BlobData;
      } u;
} ScanFixReportData;
...
typedef struct {
    ...
    LONG          MinimumBlackOnEdges; /* Minimum horizontal distance between edge of zone and text, in pixels */
    LONG          Density; /* Maximum number of white pixels per 100 black pixels */
    } SCANFIX_SC18;
...
typedef struct
{
    LONG FeedbackLinesRemoved; /* returns the number of rows/columns removed during registration */
    LONG ResultantMargin; /* Desired left/top margin */
    LONG SkipDistance; /* number of pixels in from edge to skip before looking for data */
    LONG MinBackground; /* pixel size of noise allowable in the background */
    LONG MinForeground; /* pixel size of noise allowable in the foreground */
    LONG MinActivity; /* sets the level of data needed to be considered "real" data */
    LONG ToLineMinLineLength; /* minimum line length in pixels for register to line */
    LONG ToLineMaxLineGap; /* maximum gap width in pixels for register to line */
    LONG ToLineMaxLineThickness; /* maximum thickness in pixels for register to line */
    BOOL Active; /* set to true if registration in the given axis is to be used */
    BOOL CentralFocus; /* set to true if registration should concentrate on the center */
    BOOL AddOnly; /* set true if data is not to be removed from the image */
    BOOL IgnoreHoles; /* set true to ignore hole punch marks in margin */
    BOOL SkipActive; /* set true if SkipDistance is to be used */
    BOOL ToLineActive; /* set true if register to line is to be used */
    BOOL ToLineCheckThinLinesActive; /* set true if thin lines are to be considered */
    LONG Reserved1;
} SC24_REGISTER_PARAMS;

typedef struct {
    SC24_REGISTER_PARAMS HorizontalRegister; /* parameters used for horizontal registration */
    SC24_REGISTER_PARAMS VerticalRegister; /* parameters used for vertical registration */
    } SCANFIX_SC24;

/* Virtual Bulb flags */
#define SCANFIX_VBULB_ADAPT         0x01
#define SCANFIX_VBULB_BRIGHTNESS    0x02

/* Color region flags (image detergent and color drop) */
#define SCANFIX_COLOR_ADAPT         0x01

/* brightness and contrast analysis */
#define SCANFIX_OK_CONTRAST         0x01
#define SCANFIX_BAD_CONTRAST        0x02
#define SCANFIX_ADAPTIVE_CONTRAST   0x03
#define SCANFIX_GLOBAL_CONTRAST     0x04

/* brightness and contrast correction */
#define SCANFIX_NO_FIX_CONTRAST         0x01
#define SCANFIX_AUTO_CONTRAST           0x02
#define SCANFIX_FORCE_ADAPTIVE_CONTRAST 0x03
#define SCANFIX_FORCE_GLOBAL_CONTRAST   0x04

/* variable size filters */
#define SCANFIX_FILTER_GAUSSIAN     0x01 /* variable size Gaussian smoothing filter */
#define SCANFIX_FILTER_UNSHARP      0x02 /* variable size unsharp mask sharpening filter */
#define SCANFIX_FILTER_MEDIAN       0x03 /* variable size median filter */
#define SCANFIX_FILTER_DILATE       0x04 /* variable size dilate/grow light filter */
#define SCANFIX_FILTER_ERODE        0x05 /* variable size erode/grow dark filter */
#define SCANFIX_FILTER_OPEN         0x06 /* variable size open light filter */
#define SCANFIX_FILTER_CLOSE        0x07 /* variable size close light filter */
#define SCANFIX_FILTER_AVERAGE      0x08 /* variable size box or average filter */
#define SCANFIX_FILTER_USER         0x09 /* variable size user defined convolution filter */

/* fixed size filters */
#define SCANFIX_FILTER_GAUSSIAN3X3  0x11 /* 3 by 3 pixel Gaussian smoothing filter */
#define SCANFIX_FILTER_GAUSSIAN5X5  0x12 /* 5 by 5 pixel Gaussian smoothing filter */
#define SCANFIX_FILTER_LAPLACIAN3X3 0x13 /* 3 by 3 pixel Laplacian edge detection filter */
#define SCANFIX_FILTER_PREWITTV3X3  0x14 /* 3 by 3 pixel Pewitt vertical edge detection filter */
#define SCANFIX_FILTER_PREWITTH3X3  0x15 /* 3 by 3 pixel Pewitt horizontal edge detection filter */
#define SCANFIX_FILTER_SOBELV3X3    0x16 /* 3 by 3 pixel Sobel vertical edge detection filter */
#define SCANFIX_FILTER_SOBLEH3X3    0x17 /* 3 by 3 pixel Sobel horizontal edge detection filter */

/* reasons image may not be color */
#define SCANFIX_DETECT_COLOR_UNSATURATED 0x00000001
#define SCANFIX_DETECT_COLOR_NO_ITEMS    0x00000004
#define SCANFIX_DETECT_COLOR_FEW_ITEMS   0x00000008
#define SCANFIX_DETECT_COLOR_SMALL_ITEMS 0x00000010

/* reasons image may be color */
#define SCANFIX_DETECT_COLOR_SATURATED   0x00000100
#define SCANFIX_DETECT_COLOR_FOUND_ITEMS 0x00000200

/* other status */
#define SCANFIX_DETECT_COLOR_IGNORED     0x00010000


typedef struct {
    DWORD Radius; /* radius of effect, in colorpsace units */
    DWORD Flags; /* flags describing how the color is to be handled */
    RGBQUAD Color; /* the RGB colorspace value */
} SCANFIX_COLOR_REGION;

typedef struct {
    DWORD Radius; /* radius of effect, in colorspace units */
    DWORD Flags; /* flags indicating how the hue is to be handled */
    BYTE Hue; /* the HSV colorspace hue value; 0 is red, 85 green, 171 blue */
    BYTE Reserved1;
    BYTE Reserved2;
    BYTE Reserved3;
} SCANFIX_HUE;
...
typedef struct  {
    /* inputs for analysis, used during REQ_INIT */
    BYTE TargetBrightness; /* desired minimum of background brigntness */
    BYTE TargetContrast; /* desired minimum of contrast between foreground/background */
    BYTE TargetVariance; /* desired maximum variance in background color */
    BYTE Reserved1;

    /* inputs for correction, used during REQ_EXEC */
    DWORD Correction; /* correction to be applied: none, auto, force global, force adaptive */
    LONG MinPercentile; /* the minimum percentile; all pixels below become black */
    LONG MaxPercentile; /* the minimum percentile; all pixels above become white */
    BYTE ContrastLimitDark; /* maximum change allowed in the contrast of any given dark pixel */
    BYTE ContrastLimitMid; /* maximum change allowed in the contrast of any given medium brightness pixel */
    BYTE ContrastLimitLight; /* maximum change allowed in the contrast of any given bright pixel */
    BYTE Reserved2;

    /* analysis outputs, generated by REQ_INIT */
    DWORD Method;        /* OK, bad, adaptive, or global */
    /* the following field is allocated by the opcode during REQ_INIT, and freed during REQ_TERM
       the number of elements is always 101, with element 0 representing the  0th percentile
       (the darkest pixel found) to element 100, representing the 100th percentile (the brightest) */
    BYTE PICHUGE *PercentileGraph; /* returns a graph showing the brightness for each percentile of the image */
    BYTE AverageBackground; /* returns the average background color */
    BYTE BackgroundVariance; /* returns the variance in background across the image */
    BYTE NoiseRadius; /* returns the amount of local variance in background brightness */
    BYTE AverageContrast; /* returns the average amount of contrast between foreground and background */
} SCANFIX_SC26;

typedef struct {
    DWORD ColorCount; /* number of colors in the dropout list */
    RGBQUAD DropToColor; /* color pixels will change to */
    BOOL Extract; /* set true to extract chosen color and drop everything else */
    /* the following field, if used, should be allocated by the user before REQ_INIT
       and freed after REQ_TERM; set to NULL if not needed */
    SCANFIX_COLOR_REGION PICHUGE *ColorList; /* user allocated list of colors to drop/extract */
} SCANFIX_SC29;

typedef struct {
    DWORD Type; /* Set to any SCANFIX_FILTER_xxx value */
    DWORD Width; /* width for a variable sized filter */
    DWORD Height; /* height for a variable sized filter */
    LONG CenterX; /* horizontal center point for a variable sized filter */
    LONG CenterY; /* vertical center point for a variable sized filter */
    LONG Divisor; /* divisor for user defined convolution filter */
    LONG Offset; /* offset for user defined convolution filter */
    /* the following field, if used, should be allocated by the user before REQ_INIT
       and freed after REQ_TERM; set to NULL if not needed */
    LONG PICHUGE *Weights; /* table of weights for user defined convolution filter */
} SCANFIX_SC30;
...
typedef struct {
    ...
    /* the following field, if used, should be allocated by the user before REQ_INIT
       and freed after REQ_TERM; set to NULL if not needed */
    SCANFIX_COLOR_REGION PICHUGE *ColorList;
} SCANFIX_SC31;
...
typedef struct {
    ...
    /* the following field, if used, should be allocated by the user before REQ_INIT
       and freed after REQ_TERM; set to NULL if not needed */
    SCANFIX_HUE PICHUGE *HueList;
} SCANFIX_SC32;

typedef struct {
    /* inputs */
    BOOL IgnorePaperColor; /* set to true to ignore pixels near the dominant hue of the image */
    DWORD MinimumSize; /* minimum pixel size, width or height, to be considered a significant object */
    BOOL GenerateBoxes; /* set to true to generate color object bounding box list */
    BYTE BoxOverlap; /* percent of overlap allowed before intersecting bounding boxes are merged */
    BYTE MinimumSaturation; /* minimum saturation required for a pixel to be considered color */
    BYTE MinimumBrightness; /* minimum brightness required for a pixel to be considered color */
    BYTE Sensitivity; /* value indicating how sensitive the algorithm is; 100 for a single object, 0 for thousands */
    /* outputs */
    BOOL IsColor; /* set to true if the image is color */
    DWORD Reason; /* bitfield set with SCANFIX_DETECT_COLOR_xxx flags explainign the result */
    LONG BigObjects; /* count of objects over minimum size found */
    LONG SmallObjects; /* count of objects near, but below minimum size found */
    LONG AverageSaturation; /* averate saturation of the image */
    DWORD BoxCount; /* count of large object bounding boxes */
    /* the following field is allocated by the opcode during REQ_EXEC, and freed during REQ_TERM
       element count is BoxCount * 4 */
    LONG PICHUGE *BoxCoords;  /* x, y, w, h, ... */
} SCANFIX_SC33;

typedef struct  {

    /* Outputs from correction, indicates the values used during REQ_EXEC      */
    /* For some images, two passes may be made through REQ_EXEC. In this case, */
    /* both sets of correction settings are returned.                          */
    DWORD MethodUsed;   /* SCANFIX_OK_CONTRAST, SCANFIX_BAD_CONTRAST, SCANFIX_ADAPTIVE_CONTRAST, or SCANFIX_GLOBAL_CONTRAST */
    LONG MinPercentileUsed; /* the minimum percentile; all pixels below become black */
    LONG MaxPercentileUsed; /* the minimum percentile; all pixels above become white */
    BYTE ContrastLimitDarkUsed; /* maximum change allowed in the contrast of any given dark pixel */
    BYTE ContrastLimitMidUsed; /* maximum change allowed in the contrast of any given medium brightness pixel */
    BYTE ContrastLimitLightUsed; /* maximum change allowed in the contrast of any given bright pixel */
    BYTE Reserved1;
    DWORD MethodUsed2;   /* SCANFIX_OK_CONTRAST, SCANFIX_BAD_CONTRAST, SCANFIX_ADAPTIVE_CONTRAST, or SCANFIX_GLOBAL_CONTRAST */
    LONG MinPercentileUsed2; /* the minimum percentile; all pixels below become black */
    LONG MaxPercentileUsed2; /* the minimum percentile; all pixels above become white */
    BYTE ContrastLimitDarkUsed2; /* maximum change allowed in the contrast of any given dark pixel */
    BYTE ContrastLimitMidUsed2; /* maximum change allowed in the contrast of any given medium brightness pixel */
    BYTE ContrastLimitLightUsed2; /* maximum change allowed in the contrast of any given bright pixel */
    BYTE Reserved2;
...
typedef struct{ 
   SC53_BOUNDING_BOX  PICHUGE* PunchLocation;
   LONG          PICHUGE* PunchConfidence; 

}SCANFIX_SC53;

typedef struct {
    LONG          X;        /* x coordinate of upper left corner of area  */
    LONG          Y;        /* y coordinate of upper left corner in area  */
    LONG          Width;    /* width of area  */
    LONG          Height;   /* height of area */
    } SC54_INVERSE_REGION; 

typedef struct {
    BOOL          PerformCorrection;     /* Set to TRUE to correct detected inverse areas.  If set to FALSE,
                                            list of inverse areas will be reported, but they will not
                                            be corrected. */
    SC54_INVERSE_REGION *InverseRegions; /* Op-code allocated list of Inverse Regions detected. */
                                         /* Number of regions in list indicated by CountOfFeaturesFound */
    } SCANFIX_SC54;

typedef struct {
    LONG            DegreesToRotate;     /* Number of degrees image should be rotated counterclockwise to correct */
    BOOL            DisallowSideways;   /* Force result to be 0 or 180 degrees */
    } SCANFIX_SC58;
...
typedef struct {
        DWORD         Reserved0;
        ScanFixReportData PICHUGE* ReportingData;  /* set to NULL to skip defer callback,
                        point to zeroed out structure and defer callback will be called with RES_QUERY */
        BYTE PICHUGE* Reserved2;
        BYTE PICHUGE* Reserved3;
        BYTE PICHUGE* Reserved4;
        BYTE PICHUGE* Reserved5;    /* Must be NULL! */
        ...
        LONG          Subcode;      /* 0 = detect skew */
                                    ...
                                    /* 26 = brightness and contrast correction */
                                    /* 29 = color dropout */
                                    /* 30 = filters */
                                    /* 31 = Image Detergent */
                                    /* 32 = Virtual Bulb color drop */
                                    /* 33 = color dection */
                                    /* 51 = auto-despeckle */
                                    /* 52 = auto-brightness/contrast */
                                    /* 53 = auto-hole punch removal */
                                    /* 54 = auto-inverse text correction */
                                    /* 58 = auto-orient-detect */
                                    /* 55 = auto-color-despeckle */
        LONG          Stride; /* Input stride */
        BITMAPINFOHEADER BiOut; /* Output Image Width, Height, etc. */
        RGBQUAD       PadColor; /* Color to be used for pixels beyond the bounds of the source image (subcodes in: 3, 11, 21, 24) */
        LONG          OutputStride; /* Output stride (subcodes out: all) */
        LONG          Confidence; /* Confidence that the detection process produced accurate results, between 0 and 100 (subcodes out: 0, 20, 33) */
...
#define SF_SUBCODE_CORRECT_CONTRAST       (26)
#define SF_SUBCODE_COLORDROP              (29)
#define SF_SUBCODE_FILTER                 (30)
#define SF_SUBCODE_IMAGEDETERGENT         (31)
#define SF_SUBCODE_VIRTUALBULB            (32)
#define SF_SUBCODE_DETECT_COLOR           (33)
#define SF_SUBCODE_AUTO_DESPECKLE         (51)
#define SF_SUBCODE_AUTO_CORRECT_CONTRAST  (52)
#define SF_SUBCODE_AUTO_REMOVE_HOLEPUNCHES (53)
#define SF_SUBCODE_AUTO_CORRECT_INVERSE_TEXT (54)
#define SF_SUBCODE_AUTO_COLOR_DESPECKLE   (55)
#define SF_SUBCODE_AUTO_ORIENT_DETECT     (58)
...
#define J2K_COLOR_SPEC_METHOD_ECS    (1)
#define J2K_COLOR_SPEC_METHOD_ICP    (2)

#define J2K_COLOR_SPEC_ECS_RGB       (1)  /* sRGB as defined by IEC 61966-2-1 and ISO/IEC 15444-1:2004 */
#define J2K_COLOR_SPEC_ECS_GREYSCALE (2)  /* greyscale as defined by IEC 61966-2-1 and ISO/IEC 15444-1:2004 */
#define J2K_COLOR_SPEC_ECS_GRAYSCALE J2K_COLOR_SPEC_ECS_GREYSCALE  /* alternate spelling for consistency */
#define J2K_COLOR_SPEC_ECS_YCC       (3)  /* sYCC as defined by IEC 61966-2-1 Amd. 1 and ISO/IEC 15444-1:2004 */

typedef struct {
        DWORD         Reserved0;
        BYTE PICHUGE* UpdatedTiles; /* if <> 0, points to a bitmap of updated 64x64 pixel boxes in raster order
                                        ordinarily this would be set by OP_RIDP */
        BYTE PICHUGE* ColorSpecICCProfileData;     /* Color Specification Box ICC Profile Data */
        BYTE PICHUGE* Reserved3;    /* Must be NULL! */
        BYTE PICHUGE* Reserved4;    /* Must be NULL! */
        BYTE PICHUGE* Reserved5;    /* Must be NULL! */
        BYTE PICHUGE* Reserved6;    /* Must be NULL! */
        ...
        BYTE        ColorSpecMethod;               /* Color Specification Box Method */
        SBYTE       ColorSpecPrecedence;           /* (expand only) Color Specification Box Precedence, must be 0 */
        BYTE        ColorSpecApproximation;        /* (expand only) Color Specification Box Approximation, must be 0 */
        BYTE        ColorSpecEnumeratedColorSpace; /* (expand only) Color Specification Box Enumerated Color Space */
        DWORD       ColorSpecICCProfileLen;        /* Color Specification Box ICC Profile Length */
        DWORD       CompFileSizeHigh;
    } J2K_UNION;

typedef struct {
        DWORD       ExtraSize;                     /* Size of this structure (bytes) */
        DWORD       Pad1; // so following is 8-aligned
        double      CaptureResVert;                /* Vertical Capture Resolution in canvas grid points/meter */
        double      CaptureResHorz;                /* Horz ^^ */
        double      DisplayResVert;                /* Vertical Display Resolution in canvas grid points/meter */
        double      DisplayResHorz;                /* Horz ^^ */
#if defined(__SPARC__) && defined(PIC64)
    } J2K_EXTRA __attribute__((aligned(8))); //Ensure 8-byte alignment of J2K_EXTRA variables
#elif defined(__SPARC__)
    } J2K_EXTRA __attribute__((aligned(4))); //Ensure 4-byte alignment of J2K_EXTRA variables
#else
    } J2K_EXTRA;
#endif

#define PF2_ReducedMemoryMode       (0x01000000L)
        /* (JPEG 2000) using reduced memory mode */
#define PF2_3D_Slices               (0x02000000L)
...
#define PF2_FavorSpeed              (0x10000000L)
        /* use fastest coding mode for encoding; files created using this flag will also decode faster. */
        /* for best results, also set PF2_EarlyOutRateControl. */
        /* note that when this flag is set, lossless encoded files may become slightly larger */
        /* and lossy rate-controlled encoded files may decode to slightly lower quality */

// Note: make sure additional PF2_* flags don't conflict with JPIP_UNION PF2_* flags in case we need to share at some point
//       but be aware that PF2_FavorSpeed (used only in J2K_UNION) overloads PF2_AnotherJPIPResponseAvailable (used only in JPIP_UNION)

#define PF2_UseResSeek              (0x00000400L)
#define PF2_AllowResSeek            PF2_UseResSeek
#define PF2_P4OptDisable            (0x00000800L)    // disable P4 optimizations
#define PF2_ASMDisable_All          (0x00000960L)    // disable all assembly optimizations
                                                     // equivalent to: ( PF2_MMXDisable | PF2_P2AndP3OptDisable |
                                                     //                  PF2_P3OptDisable | PF2_P4OptDisable )
...
#define BIN_MODE_AUTO           3
#define BIN_MODE_AUTO_SUBCODE_USE_GLOBAL_THRESHOLD 4
...
#define HDP_SS_NONE (0)
#define HDP_SS_400 (1)
#define HDP_SS_420 (2)
#define HDP_SS_420_COSITED (2)
#define HDP_SS_420_CENTERED (5)
#define HDP_SS_420_OTHER (7)
#define HDP_SS_422 (3)
#define HDP_SS_422_COSITED (3)
#define HDP_SS_422_CENTERED (6)
#define HDP_SS_422_OTHER (8)
#define HDP_SS_444 (4)
...
/* These are the allowable values for the enhancement-mode field of the HDP_UNION stucture */
#define HDP_OPTIMIZE_NONE         (0)
#define HDP_OPTIMIZE_FOR_VISUAL   (1)
#define HDP_OPTIMIZE_FOR_SSIM     (2)
#define HDP_OPTIMIZE_FOR_SNR      (3)

#define PF2_HardTileEdges    (0x00000001L)
#define PF2_SkipJPEGXRHeader (0x00000002L)
#define PF2_JPEGXR_Format    (0x00000008L)
...
typedef struct {
        ...
        DWORD          Enhancements;          /* enhancement modes, see defines above */
    } HDP_UNION;
...
typedef struct PIC_PARM_TAG {
...
#if (defined(__SPARC__) || defined(__AIX64__)) && defined(PIC64)
        DWORD         Pad1; // so following is 8-aligned
#endif
        ...
        BYTE PICHUGE* opcodeExtraPtr; // if not NULL must point to a *_EXTRA struct
        ...
        PICINTPTRT    tlsReserved7;    /* reserved picwin32.c */
        PICINTPTRT    Reserveds[87];
#if (defined(__SPARC__) || defined (__AIX64__))&& defined(PIC64)
...
#if 0
/* all PF2_ flags repeated here in numerical order */
#define PF2_ActOnRectangle                (0x00000001L)
#define PF2_ClipDc                        (0x00000001L)
#define PF2_Cosited                       (0x00000001L)
#define PF2_DarkSpeck                     (0x00000001L)
#define PF2_ForceWSQv2Compliance          (0x00000001L)
#define PF2_FoundDicom                    (0x00000001L)
#define PF2_HardTileEdges                 (0x00000001L)
#define PF2_HasThumbnail                  (0x00000001L)
#define PF2_InvertAlpha                   (0x00000001L)
#define PF2_IsExif                        (0x00000001L)
#define PF2_IsInterlacedGIF               (0x00000001L)
#define PF2_IsKeyFrame                    (0x00000001L)
#define PF2_JBIG2Transcode_ForEmbeddedPDF (0x00000001L)
#define PF2_MakeCorrectTiffJPEG           (0x00000001L)
#define PF2_ShowReduceError               (0x00000001L)
#define PF2_SkipJP2Header                 (0x00000001L)
#define PF2_AllowJBIG2Globals             (0x00000002L)
#define PF2_ApplyReduceErrorThreshold     (0x00000002L)
#define PF2_AutoColor                     (0x00000002L)
#define PF2_ClipAc                        (0x00000002L)
#define PF2_ForceKeyFrame                 (0x00000002L)
#define PF2_ForceLossless                 (0x00000002L)
#define PF2_GIFColorsSorted               (0x00000002L)
...
#define PF2_SmoothUpsample                (0x00000002L)
#define PF2_565Output                     (0x00000004L)
#define PF2_AutoLum                       (0x00000004L)
#define PF2_ColorMap444                   (0x00000004L)
#define PF2_FastDecompress                (0x00000004L)
#define PF2_Hurry                         (0x00000004L)
#define PF2_JPEGXR_Format                 (0x00000004L)
#define PF2_KeepColorTable                (0x00000004L)
#define PF2_LocalGIFColors                (0x00000004L)
#define PF2_SpecialTIFF                   (0x00000004L)
#define PF2_UseISOStandardHuffmanTbls     (0x00000004L)
#define PF2_AutoQuantizationThreshold     (0x00000008L)
#define PF2_ClippedYUV                    (0x00000008L)
#define PF2_ComputeContrastFixedPoint     (0x00000008L)
#define PF2_GIFAllow1and4Bpp              (0x00000008L)
#define PF2_OmitISOStandardHuffmanTbls    (0x00000008L)
#define PF2_ReachedEOI                    (0x00000008L)
#define PF2_SkipUpsampling                (0x00000008L)
...
#define PF2_AlternateRateDistortion       (0x00000010L)
#define PF2_CompressedYUV                 (0x00000010L)
#define PF2_FrameByFrameQthr              (0x00000010L)
#define PF2_PCXGetResolution              (0x00000010L)
#define PF2_UseContrastFixedPoint         (0x00000010L)
#define PF2_P2AndP3OptDisable             (0x00000020L)
#define PF2_ARMNeonDisable                (0x00000020L)
#define PF2_P3OptDisable                  (0x00000040L)
#define PF2_UseResPoke                    (0x00000080L)
#define PF2_MMXDisable                    (0x00000100L)
#define PF2_ARMAsmDisable                 (0x00000100L)
#define PF2_OmitPICMetaData               (0x00000200L)
#define PF2_UseResSeek                    (0x00000400L)
#define PF2_P4OptDisable                  (0x00000800L)
#define PF2_RetrieveAlpha                 (0x00001000L)
#define PF2_SaveAlpha                     (0x00001000L)
#define PF2_HaveAlpha                     (0x00002000L)
#define PF2_RGBAIsPremultiplied           (0x00004000L)
#define PF2_RetrieveRGBANonPremultiplied  (0x00008000L)
#define PF2_SSSE3OptDisable               (0x00010000L)
#define PF2_ReducedMemoryMode             (0x01000000L)
#define PF2_3D_Slices                     (0x02000000L)
#define PF2_InsertPPMMarker               (0x04000000L)
#define PF2_HexHexEncode                  (0x08000000L)
#define PF2_AnotherJPIPResponseAvailable  (0x10000000L)
#define PF2_ExifInputIsBigEndian          (0x10000000L)
#define PF2_FavorSpeed                    (0x10000000L)
#define PF2_AcrobatCmyk                   (0x20000000L)
#define PF2_EarlyOutRateControl           (0x20000000L)
#define PF2_AllowOutOfRangeBits           (0x40000000L)
#define PF2_InsertTLMMarker               (0x40000000L)
#define PF2_RgbaOutput                    (0x40000000L)
#define PF2_UseAlphaMask                  (0x40000000L)
#define PF2_NoYccTransform                (0x80000000L)
#define PF2_RgbJpeg                       (0x80000000L)
#define PF2_SwapBW                        (0x80000000L)
#define PF2_Use32BitPrecision             (0x80000000L)
...
/* alternate PF2_ names */
#define PF2_AllowResSeek                  PF2_UseResSeek
#define PF2_ASMDisable_All                (0x00000960L)    \\ equivalent to: ( PF2_MMXDisable | PF2_P2AndP3OptDisable |
                                                           \\                  PF2_P3OptDisable | PF2_P4OptDisable )

/* available PF2_ flags */
#                                         (0x00020000L)
#                                         (0x00040000L)
#                                         (0x00080000L)
#                                         (0x00100000L)
