...
/***************************************************************************\
*                                                                           *
* Copyright (C) 1993-2012 Accusoft Corporation  All rights reserved.        *
*                                                                           *
\***************************************************************************/
...
typedef size_t    PICSIZET;
    ...
    #define PICHAVELONGLONG
#elif defined(LLONG_MAX) || defined(__GNUC__)
    typedef long long PICLONGLONG;
    typedef unsigned long long PICULONGLONG;
    #define PICHAVELONGLONG
#else
    // else we'll get a compile error from any opcode that uses
    // these 64-bit types.  So we don't want to use them in source
    // or includes shared across opcodes without some #define
    // to positively enable their use so the default is they aren't
    // used.
#endif
    ...
    #define EXPORTWINAPI
    #if defined(PIC64)
        #define DLLEXPORTWINAPI __attribute__ ((visibility("default")))
    #else
    ...
    #endif
    #define WINAPI
    #define EXPORTWINAPI __attribute__ ((visibility("default")))
    #define DLLEXPORTWINAPI __attribute__ ((visibility("default")))
    #define PICHUGE
    #define PICFAR
    #define PICFARFN
    ...
    /* Note the following is also used for iOS */
...
#if defined(PICSEEK64)

#if !defined(PICHAVELONGLONG)
// PICSEEK64 requires a 64-bit portable type
#error "PICSEEK64 #define'd in a compiler environment with neither <long long> nor <__int64>"
#endif

    typedef PICULONGLONG PICOFST;
    typedef PICLONGLONG PICOFSDIFFT;

#else   // !defined(PICSEEK64)

    typedef PICSIZET PICOFST;
    typedef PICPTRDIFFT PICOFSDIFFT;
